...
class WordsDatabase:
    ...
    def main_tables_exist(self):
        self.connect()
        try:
            self.cursor.execute("""
            SELECT * FROM MAIN_WORDS, MAIN_WORDS_VARIANTS;
            """)
            return True
        except UndefinedTable:
            print("Db initializing first time")
            return False
    ...
    def get_random_word(self, chat_id) -> tuple | None:
        if (
            len(self.remaining_main_words[chat_id] + self.remaining_user_words[chat_id])
            == 1
        ):
            self.cycle = False
        if self.remaining_main_words[chat_id] or self.remaining_user_words[chat_id]:
            random_word = random.choice(
                self.remaining_main_words[chat_id] + self.remaining_user_words[chat_id]
            )
